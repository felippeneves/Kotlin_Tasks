...
class RegisterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun create(name: String, email: String, password: String) {
        ...
        if(name.isBlank())
        {
            mCreate.value = ValidationListener(mContext.getString(R.string.enter_name))
            return
        }

        if(email.isBlank())
        {
            mCreate.value = ValidationListener(mContext.getString(R.string.enter_email))
            return
        }

        if(password.isBlank())
        {
            mCreate.value = ValidationListener(mContext.getString(R.string.enter_password))
            return
        }
